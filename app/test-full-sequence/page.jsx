...
export default function TestFullSequencePage() {
  ...
  useGSAP(() => {
    ...
    const sTl = gsap.timeline({
      scrollTrigger: {
        ...
        invalidateOnRefresh: true,
        onUpdate: (self) => {
          // Mecanismo de entrega de controle (Fase 5 Proof)
          // Se o progresso chegar no final (ex: 95%), ativa a seção final
    ...
    // Pequeno respiro final no pin antes de soltar
    sTl.to({}, { duration: 0.5 });
  ...
  return (
    <div ref={containerRef} className="bg-black text-white overflow-x-hidden font-sans">
        ...
        <div className="w-full md:w-1/2 h-1/2 md:h-full flex flex-col justify-center px-10 md:px-20 z-20 bg-black border-b md:border-b-0 md:border-r border-white/5">
          <div className="max-w-md">
            ...
            <p className="text-lg md:text-xl text-[#acaba9] mb-12 font-light leading-relaxed">
              Soluções que unem estética refinada e engenharia de alta performance.
            </p>
            <div className="flex gap-12 mb-16">
              <div>
                <span className="text-4xl md:text-5xl font-bold text-white block mb-1">35+</span>
                <span className="text-xs uppercase tracking-widest text-[#acaba9]">anos</span>
              </div>
              <div>
                <span className="text-4xl md:text-5xl font-bold text-white block mb-1">7.040+</span>
                <span className="text-xs uppercase tracking-widest text-[#acaba9]">projetos</span>
              </div>
            </div>
            <BlurTextReveal
              text="Selected Work"
      ...
      {/* BLOCO FINAL: Simulação do Services.jsx (Entrega de Controle) */}
      <section className="relative min-h-screen bg-black flex flex-col items-center justify-center px-10">
        <AnimatePresence>
          {showFinalSection && (
            <motion.div
              ...
              exit={{ opacity: 0, y: 30 }}
              transition={{ delay: 0.8, duration: 0.8, ease: "easeOut" }} // Delay de 800ms de leitura simulado
              className="text-center"
            >
              <h2 className="text-5xl md:text-8xl font-black uppercase text-white mb-4">Nossos Serviços</h2>
              <span className="text-xl md:text-2xl uppercase tracking-[0.5em] text-[#acaba9] font-light">Alto Padrão</span>
              <div className="mt-20 p-10 border border-white/10 rounded-xl bg-zinc-900/50">
                <p className="text-[#acaba9] text-sm uppercase tracking-widest">
                  Este bloco simula o Services.jsx real.<br/>
                  O delay de 800ms foi respeitado.<br/>
                  Se rolar pra cima, eu sumo instantaneamente.
                </p>
              </div>
            </motion.div>
          )}
      ...
      <div className="h-screen bg-zinc-950 flex items-center justify-center">
        <h1 className="text-4xl font-light uppercase tracking-widest opacity-20">Fim do Teste</h1>
      </div>
    </div>
  );
}
