export const TestimonialsData = [
  {
    id: "luxury-presence",
    companyName: "Luxury presence",
    clientName: "Malte Smith",
    clientDeg: "Founder & CEO · USA",
    clientImage: "/images/placeholder.webp",
    quoteMessage:
      "A polished, collaborative experience that helped us present the brand with more confidence.",
    videoURL: "https://www.youtube.com/embed/rOAsYNtPAmQ?autoplay=1",
  },
  {
    id: "credible",
    companyName: "credible",
    clientName: "Stephen Dash",
    clientDeg: "Founder & CEO · USA",
    clientImage: "/images/placeholder.webp",
    quoteMessage:
      "Clean execution, fast turnaround, and a level of attention that made everything easier.",
  },
  {
    id: "fast-resume",
    companyName: "Fast resume",
    clientName: "Doug Petrie",
    clientDeg: "Founder & CEO · USA",
    clientImage: "/images/placeholder.webp",
    quoteMessage:
      "The final result felt premium, direct, and fully aligned with what we needed.",
    videoURL: "https://www.youtube.com/embed/eKB_kigzDwA?autoplay=1",
  },
  {
    id: "technis",
    companyName: "Technis",
    clientName: "Jean-Baptiste Biolay",
    clientDeg: "General Manager · UAE",
    clientImage: "/images/placeholder.webp",
    quoteMessage:
      "A thoughtful process and a sharp result that elevated the entire experience.",
  },
  {
    id: "ventigence",
    companyName: "Ventigence",
    clientName: "Zoltan Csonka",
    clientDeg: "Founder & CEO · UAE",
    clientImage: "/images/placeholder.webp",
    quoteMessage:
      "Exactly the kind of refined visual language we wanted for the project.",
    videoURL: "https://www.youtube.com/embed/9F4WsbJ1mrc?autoplay=1",
  },
];
