// Owns canvas sizing, DPR and input-capability detection without rebuilding the scene model itself.
export default class ViewportController {
  ...
  update() {
    ...
    const dpr = Math.min(window.devicePixelRatio || 1, this.config.performance.maxDevicePixelRatio);
    ...
    console.info("[HeroScene] resize detected", {width, height, dpr, profile, capabilities});
